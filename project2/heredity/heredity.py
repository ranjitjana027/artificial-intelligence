    ...
    for person in people.keys():
        if people[person]['mother'] == None and people[person]['father']==None:
            ...
            if person in have_trait:
                p*=(PROBS['gene'][gene]* PROBS['trait'][gene][True])
            else:
                p*=(PROBS['gene'][gene]*PROBS['trait'][gene][False])
        else:
            m=PROBS['mutation']
            f=PROBS['mutation']
            ...
            if person in two_genes:
                p*=m*f*PROBS['trait'][2][trait]
            elif person in one_gene:
                p*=((m*(1-f))+(f*(1-m)))*PROBS['trait'][1][trait]
            else:
                p*=((1-m)*(1-f))*PROBS['trait'][0][trait]
    return p
...
def normalize(probabilities):
    ...
    """
    for person in probabilities.keys():
        sum=probabilities[person]['gene'][1]+probabilities[person]['gene'][2]+probabilities[person]['gene'][0]
        for k in probabilities[person]['gene'].keys():
            probabilities[person]['gene'][k]/=sum
